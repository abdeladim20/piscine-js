export function filterCurry(predicate) {
    return (obj) => Object.fromEntries(Object.entries(obj).filter(([key, value]) => predicate([key, value])))
}
...
export function reduceCurry(reduce1) {
    return (obj, initialValue) => Object.entries(obj).reduce((acc, [key, value]) => reduce1(acc, [key, value]),initialValue)
}

export function reduceScore(entries, initialValue = 0) {
    return reduceCurry((acc, [, v]) => v.isForceUser ? acc + v.pilotingScore + v.shootingScore : acc)(entries, initialValue)
}
...
export function mapCurry(func) {
    return (obj) => Object.fromEntries(Object.entries(obj).map(([key, value]) => func([key, value])))
}